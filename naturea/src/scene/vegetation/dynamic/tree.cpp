...
/* Constructor */
Tree::Tree(TextureManager *texManager, ShaderManager *shManager):SceneModel(texManager, shManager){
	dataTexture		= NULL;
	lColorTexture	= NULL;
	bColorTexture	= NULL;
...
void Tree::init(){
	...
	locations[TREE_NORMAL]				= glGetAttribLocation(branchShaderID, "normal");
	locations[TREE_TANGENT]				= glGetAttribLocation(branchShaderID, "tangent");
	locations[TREE_XVALS]				= glGetAttribLocation(branchShaderID, "x_vals");
	locations[TREE_BRANCH_INDEX]        = glGetAttribLocation(branchShaderID, "branch_index");
	locations[TREE_BRANCH_DATA_TEXTURE] = glGetUniformLocation(branchShaderID, "data_tex");
	locations[TREE_TIME]				= glGetUniformLocation(branchShaderID, "time");
	locations[TREE_BRANCH_COUNT]		= glGetUniformLocation(branchShaderID, "branch_count");
	locations[TREE_NOISE_TEXTURE0]		= glGetUniformLocation(branchShaderID, "branch_noise_tex");
	locations[TREE_NOISE_TEXTURE1]		= glGetUniformLocation(branchShaderID, "leaf_noise_tex");
	locations[TREE_WIND_DIRECTION]		= glGetUniformLocation(branchShaderID, "wind_direction");
	locations[TREE_WIND_STRENGTH]		= glGetUniformLocation(branchShaderID, "wind_strength");
	locations[TREE_WOOD_AMPLITUDE]		= glGetUniformLocation(branchShaderID, "wood_amplitude");
	locations[TREE_COLOR_TEXTURE0]		= glGetUniformLocation(branchShaderID, "color_texture");
	locations[TREE_TEXCOORD0]			= glGetAttribLocation(branchShaderID, "texCoords0");

	leafLocations[TREE_NORMAL]				= glGetAttribLocation(leafShaderID, "normal");
	leafLocations[TREE_TANGENT]				= glGetAttribLocation(leafShaderID, "tangent");
	leafLocations[TREE_XVALS]				= glGetAttribLocation(leafShaderID, "x_vals");
	leafLocations[TREE_BRANCH_INDEX]			= glGetAttribLocation(leafShaderID, "branch_index");
	leafLocations[TREE_TEXCOORD0]			= glGetAttribLocation(leafShaderID, "texCoords0");
	leafLocations[TREE_BRANCH_DATA_TEXTURE]	= glGetUniformLocation(leafShaderID, "data_tex");
	leafLocations[TREE_TIME]					= glGetUniformLocation(leafShaderID, "time");
	leafLocations[TREE_BRANCH_COUNT]			= glGetUniformLocation(leafShaderID, "branch_count");
	leafLocations[TREE_NOISE_TEXTURE0]		= glGetUniformLocation(leafShaderID, "branch_noise_tex");
	leafLocations[TREE_NOISE_TEXTURE1]		= glGetUniformLocation(leafShaderID, "leaf_noise_tex");
	leafLocations[TREE_WIND_DIRECTION]		= glGetUniformLocation(leafShaderID, "wind_direction");
	leafLocations[TREE_WIND_STRENGTH]		= glGetUniformLocation(leafShaderID, "wind_strength");
	leafLocations[TREE_WOOD_AMPLITUDE]		= glGetUniformLocation(leafShaderID, "wood_amplitude");
	leafLocations[TREE_COLOR_TEXTURE0]		= glGetUniformLocation(leafShaderID, "color_texture");
	...
	bNoiseID = textureManager->loadTexture(BRANCH_NOISE_TEXTURE, "branch_noise_tex", 1, false, GL_REPEAT, GL_LINEAR, true);

	branchNoiseTexture = new Texture();
	branchNoiseTexture->load("textures/habel/Noise_20.png",1, false, GL_REPEAT, GL_LINEAR, true);
	// add branch color texture
	bColorTexture = new Texture();
	bColorTexture->load(TEX_WOOD1, 0, true , GL_REPEAT, GL_LINEAR_MIPMAP_LINEAR);
	
	leafNoiseTexture = new Texture();
	leafNoiseTexture->load("textures/habel/Noise_53.png",2, false, GL_REPEAT, GL_LINEAR, true));

	// add leaf color texture
	lColorTexture = new Texture();
	lColorTexture->load(TEX_LEAF1, 0, true , GL_REPEAT, GL_LINEAR_MIPMAP_LINEAR);
...
void Tree::draw(){
	...
	lColorTexture->bind(GL_TEXTURE4);
